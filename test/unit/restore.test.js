...
describe("Set de pruebas para las funciones de restore", function() {
    let restore;
    ...
    it("Prueba para crear un nuevo restore", async function() {
        restore = await restoreDao.createRestore({email: "test@correo.com", createDate: "2023-07-05T23:57:02.187+00:00", expirateDate: "2023-07-06T00:57:02.187+00:00"})
        expect(restore).to.have.property("_id");
        expect(restore).to.be.a("object");
    })

    it("Prueba para traer coleccion completa de restore", async function() {
        const result = await restoreDao.getRestores();
        expect(result).to.be.a("array");
    })
    
    it("Prueba para buscar un restore por el id", async function() {
    ...
    it("Prueba para buscar un restore por el email", async function() {
        const result = await restoreDao.getRestoreByEmail(restore.email);
        expect(result).to.be.a("array");
        expect(result[0].email).to.be.eql(restore.email)
    })

    it("Prueba para eliminar restores por su email", async function() {
