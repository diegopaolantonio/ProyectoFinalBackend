...
describe("Set de pruebas unitarias para las funciones de Ticket", function () {
  let mockOrder;
  let mockTicket;
  ...
  it("Prueba unitaria para generar los datos del ticket por el DTO", async function () {
    const amount = 100;
    ...
    expect(mockOrder)
      .to.have.property("purchase_datetime")
      .and.to.be.a("String");
    expect(mockOrder).to.have.property("purchaser").and.to.be.a("String");
  });

  it("Prueba unitaria para crear un nuevo Ticket", async function () {
  ...
  it("Prueba unitaria para mostrar la coleccion completa de Tickets", async function () {
    const result = await ticketDao.getTickets();
    expect(result).to.be.a("array");
    expect(result.length).to.be.gt(0);
  });

  it("Prueba unitaria para buscar un Ticket por su Id", async function () {
    const result = await ticketDao.getTicketById(mockTicket._id);
    expect(result).to.be.a("object");
    expect(result._id).to.be.eql(mockTicket._id);
  });

  it("Prueba unitaria para eliminar un Ticket por su id", async function () {
