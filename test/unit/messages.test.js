...
describe("Set de pruebas unitarias para las funciones de mensajes", function () {
  let message;
  ...
  it("Prueba unitaria para crear nuevo mensaje", async function () {
    message = await messageDao.createMessage({
      user: "Test user",
  ...
  it("Prueba unitaria para mostrar los mensajes de la coleccion", async function () {
    const result = await messageDao.getMessages();
    expect(result).to.be.a("array");
    expect(result.length).to.be.gt(0);
  });

  it("Prueba unitaria para eliminar un mensaje por su Id", async function () {
