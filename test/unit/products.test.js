...
describe("Set de pruebas unitarias de las funciones de productos", function () {
  const mockProduct = {
    title: "Test",
  ...
  it("Prueba unitaria para crear un producto", async function () {
    product = await productDao.addProduct(mockProduct);
    expect(product).to.have.property("_id");
    expect(product.owner).to.be.eql("admin");
  });

  it("Prueba unitaria para buscar la coleccion de productos", async function () {
    const result = await productDao.getProducts();
    expect(result).to.be.a("object");
  });

  it("Prueba unitaria para buscar un producto por id", async function () {
  ...
  it("Prueba unitaria para actualizar un producto", async function () {
  ...
  it("Prueba unitaria para eliminar un producto por su id", async function () {
